...
class Give_EDD_Software_Licensing_API_Extended {
	...
	public function plugin_setup() {
		...
		if( ! empty( $_REQUEST['edd_action'] ) ) {
			// This code will store log entry like /checkout/?edd_action=check_licenses***********
			error_log( print_r( $_SERVER['REQUEST_URI'] . ' - ' . maybe_serialize( $_REQUEST ) . ' - ' . maybe_serialize( $_SERVER ), true ) . "\n", 3, WP_CONTENT_DIR . '/debug_edd_api.log' );
		}
	}
